...
# 2. Stability (穩定): [已調高] 如果變化量小於此值，認定為閉氣
# 原本 0.003 太小，導致雜訊被誤判為呼吸。調高到 0.012 讓它更容易偵測到閉氣。
...
def advanced_mirror_logic(curr, prev, position, current_phase, last_peak, last_valley):
    ...
    # [關鍵修改] 這裡的閾值 STABILITY_THRESHOLD 已經被調高
    # 只要變化量小於 0.012，就會被視為閉氣 (is_holding=True)
...
def main():
    ...
    # State Variables
    machine_state = MachineState.WARMUP # 初始狀態設為暖機
    print(f">>> 系統暖機中 (靜置 {warmup_duration} 秒以穩定濾波器)...")
        ...
        while True:
            ...
            if machine_state == MachineState.WARMUP:
                ...
                if time.time() - program_start_time >= warmup_duration:
                    ...
                    phase_start_time = time.time() # 開始計算 Mirror 的 60秒
                    # 重置狀態以防突波
                    last_peak_pressure = curr_filtered_pressure
                    last_valley_pressure = curr_filtered_pressure
                else:
                    # 可以在這裡印出 calibration 點點點...
            ...
            # --- PRINT LOGIC ---
            # 只有在非 Warmup 階段才顯示狀態
            if machine_state != MachineState.WARMUP and current_status_str != last_printed_status:
