...
# 1. Hysteresis: 防止 Mirror 階段狀態亂跳
HYSTERESIS_THRESHOLD = 0.02  
# 2. Stability: 只有在 Mirror 階段，變化小於此值時馬達會靜止
...
# --- Logic 1: MIRROR PHASE (Reactive + Hold Detection) ---
def advanced_mirror_logic(curr, prev, position, current_phase, last_peak, last_valley):
    """
    Mirror 階段邏輯：
    1. 包含 Hysteresis (遲滯比較) 以穩定狀態切換。
    2. 包含 Stability Check (閉氣偵測)：如果使用者閉氣，馬達會停止 (direction=0)。
    """
    direction = 0
    ...
    # [關鍵修改] 只有在 Mirror 階段，微小的變化會導致馬達停止
    if abs(delta) < STABILITY_THRESHOLD:
        direction = 0  # 閉氣/靜止 -> 馬達停止
    else:
        # 變化足夠 -> 追隨使用者
...
# --- Logic 2: GUIDE PHASE (Proactive + Ignore User Hold) ---
def guide_breathing_logic(machine_breath, target_breath, position):
    """
    Guide 階段邏輯：
    1. 完全依賴 Timer (machine_breath)。
    2. [關鍵] 不檢查感測器數值，不檢查閉氣。
    3. 只要時間在走，馬達就會動，強迫使用者跟隨。
    """
    direction = 0
    half_cycle = target_breath / 2.0
    
    # 純時間控制，無視 Sensor 雜訊或閉氣
...
def main():
    ...
    print(">>> Phase: MIRROR Started (Demo: 5 seconds)") 
    print("    [Info] Mirror Mode: Stops if you hold breath.")
    ...
    try:
        while True:
            ...
            if machine_state == MachineState.MIRROR:
                ...
                # Check Timeout (5.0s for Demo)
                if time.time() - phase_start_time >= 5.0:
                    print("\n" + "="*30)
                    print(">>> TIMEOUT: Switching to GUIDE Phase")
                    print("    [Info] Guide Mode: Motor moves continuously.")
                    print("="*30 + "\n")
                ...
                # [GUIDE] 呼叫純時間控制的邏輯 (無視閉氣)
                machine_breath_timer, la_position = guide_breathing_logic(
                    machine_breath_timer, target_breath_time, la_position
                )
                
                # Note: Guide 階段還是需要持續計算 User Breath 來做 Evaluation
                # 但這裡的計算純粹是為了「評估」，不影響馬達移動
                # 為了簡化 Demo Code，這裡保留原本的 Evaluation 結構
                
                # User Breath Detection for Evaluation (Reusing simple logic for data collection)
                # 這裡需要一個簡單的偵測邏輯來判斷 User 是否跟上 (不影響馬達)
                logic_phase = user_phase # Default keep same
                if curr_filtered_pressure > prev_filtered_pressure + 0.005: # Simple threshold
