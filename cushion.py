...
def validate_stable(pressures, target_breath_time, validate_count, remove_count):
    ...
    elif np.all(percentage_deviation > fail_threshold):
        next_target_breath_time = min(np.median(filtered_breath_times), target_breath_time - increase_breath_time / 2)
...
def mirror_breathing(curr_pressure, prev_pressure, position, direction, vibration_pwm):
    ...
    if (prev_pressure != 0 and curr_pressure > prev_pressure): # inhale
        if (vibrate_duty_cycle != 40):
            vibrate_duty_cycle = 40
        ...
        if (vibrate_duty_cycle != 15):
            vibrate_duty_cycle = 15
            vibration_pwm.ChangeDutyCycle(vibrate_duty_cycle)
...
def guide_breathing(machine_breath, target_breath, position):
    direction = 0
    if (machine_breath < target_breath / 2): # inhale
        if (position <= linear_actuator_max_distance):
            direction = 1
    ...
    elif (machine_breath >= target_breath / 2 and machine_breath < target_breath): # exhale
        if (position >= 0):
            direction = -1
...
def main():
    ...
    # target
    target_breath_time = 0 #DEBUG

    # machine state
    machine_state = MachineState["MIRROR"] #DEBUG
    machine_breath = 0
    curr_state_count = 0
