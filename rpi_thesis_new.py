...
percentage_deviation_threshold = 10
...
def validate_stable(pressures, target_breath_time):
    filtered_pressures = lowpass_filter(pressures, lowpass_cutoff, lowpass_fs)
    state = UserState["INHALE"]
    filtered_breath_times = []
    ...
    if (len(pressure_index) > success_window + 2):
        cutoff_index = pressure_index[-(success_window + 3)]
        pressures = pressures[cutoff_index:]
        filtered_breath_times = filtered_breath_times[:-success_window]

    percentage_deviation = np.abs((filtered_breath_times - target_breath_time) / target_breath_time) * 100

    if np.all(percentage_deviation <= percentage_deviation_threshold):
...
def main():
    ...
    while True:
        ...
        if (machine_state == MachineState["MIRROR"]):
            curr_state_count += 1
            if (len(pressures) > 1):
                la_position, la_direction = mirror_breathing(curr_pressure, pressures[-2], la_position, la_direction)
        ...
        if ((machine_state == MachineState["MIRROR"] and len(pressures) > 1 and curr_pressure > pressures[-2]) or 
            ...
            if (user_state != UserState["INHALE"]):
                ...
                if (machine_state == MachineState["MIRROR"] and curr_state_count >= 1 / sampling_rate * 60):
                    ...
                    if (guide_window_reach == False and curr_state_count >= 1 / sampling_rate * target_breath_time * success_window):
                        # AFTER 4 INITIAL GUIDE
                        pressures, user_stable = validate_stable(pressures, target_breath_time)
                        curr_state_count = 0
                        guide_window_reach = True

                    elif (guide_window_reach == True and curr_state_count >= 1 / sampling_rate * target_breath_time):
                        # AFTER 1 GUIDE
                        pressures, user_stable = validate_stable(pressures, target_breath_time)
                        curr_state_count = 0
                    ...
                    if (user_stable == True):   
                        ...
                        user_stable = False
                        guide_window_reach = False
                        target_breath_time += increase_breath_time
                        print("Target breath time: ", target_breath_time)
                        pressures = []
